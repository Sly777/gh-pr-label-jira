 ...
 */
async function _syncLabel({
  prefix,
  labels,
  githubPrNumber,
  githubClient,
  githubContext,
}: SyncLabelInput): Promise<SyncLabelOutput> {
  ...
  const existingLabelsOfType = compact(
    existingLabels
      .filter((label) =>
  ...
  core.debug(
    `Labels of type "${prefix}" currently on the PR: [${existingLabelsOfType.join(
      ","
    )}]`
  );
  core.debug(
    `Labels of type "${prefix}" that will be removed: [${labelsToRemove.join(
      ","
    )}]`
  );
  core.debug(
    `Labels of type "${prefix}" that will be added: [${labelsToAdd.join(",")}]`
  );
 ...
 */
export async function syncIssueType({
  jiraIssueDetails,
  githubPrNumber,
  githubClient,
  githubContext,
  labelPrefix,
}: OperationInput): Promise<void> {
  const prefix = labelPrefix + "Issue Type";
